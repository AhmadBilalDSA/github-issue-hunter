#!/usr/bin/env python3
"""
GitHub Issue Hunter CLI Entrypoint
"""
...
def export_markdown(issues: list, output_file: str):
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        f.write(f"> Generated: **{time.strftime('%Y-%m-%d %H:%M:%S UTC', time.gmtime())}** | Total: **{len(issues)}**\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="CLI tool to hunt open-source starter issues.")
    parser.add_argument("--output", default="target_issues.md", help="Output Markdown filepath")
    parser.add_argument("--labels", default="any", help="Comma-separated labels or 'any'")
